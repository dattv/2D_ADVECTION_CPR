...
%% Parameters
cfl = 3E-2; % CFL condition (~1.2k iterations)
tEnd= 4; % final time
  P = 2;  	% degree of accuracy
nEx = 40;   % number of elements in x direction
nEy = 40;   % number of elements in y direction

%% PreProcess
% Advection coefs: Full shear cell test
Ux = @(x,y) -sin(pi*x).*cos(pi*y);
Uy = @(x,y)  cos(pi*x).*sin(pi*y);
