...
%% Parameters
cfl = 4E-2; % CFL condition (~200 iterations)
tEnd= 0.06; % final time
  P = 04;  	% degree of accuracy
nEx = 20;   % number of elements in x direction
nEy = 20;   % number of elements in y direction

%% PreProcess
% Advection coefs: Rigid body rotation (washing machine problem ;D)
Ux = @(x,y) -6.2*ones(size(x));  
Uy = @(x,y)  5.7*ones(size(y));  
...
while t < tEnd
    ...
    for RKs = 1:5
        ...
        u = u + rk4b(RKs)*res_u;
        % Prevent Aliassing Inestability in Corners
        u = FixCorners(u,invV,nN,nE);
    end
